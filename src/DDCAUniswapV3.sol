...
import { ISwapRouter02, IQuoterV2, IPermit2 } from "./interfaces/IUniswapV3.sol";

/// @title DDCAUniswapV3 - Decentralized Dollar Cost Averaging for Uniswap V3
/// @author @MoMannn
/// @notice This contract enables automated DCA strategies on Uniswap V3 using MetaMask's Delegation Framework
contract DDCAUniswapV3 is Ownable2Step {
    ...
    /// @notice Gets expected output amount from the Uniswap V3 QuoterV2
    ...
    function _getQuote(
        ...
        (expectedOut,,,) = quoter.quoteExactInputSingle(params);
    }
    ...
    /// @notice Gets a quote for a swap including slippage calculation
    /// @param _tokenIn Input token address
    /// @param _tokenOut Output token address
    /// @param _fee Pool fee tier (500 = 0.05%, 3000 = 0.3%, 10000 = 1%)
    /// @param _amountIn Amount of input token
    /// @return expectedOut Expected output amount
    /// @return minOut Minimum output after slippage tolerance
    function getQuote(
        address _tokenIn,
        address _tokenOut,
        uint24 _fee,
        uint256 _amountIn
    ) public returns (uint256 expectedOut, uint256 minOut) {
        expectedOut = _getQuote(_tokenIn, _tokenOut, _fee, _amountIn);
    ...
    function swapByDelegation(
        ...
        (, uint256 minOut) = getQuote(_tokenIn, _tokenOut, _fee, amountAfterFee);
