...
contract DDCAUniswapV3TestSepolia is BaseTest {
    DDCAUniswapV3NoQuoter public ddca;

    // Uniswap V3 SwapRouter02 address on Sepolia
    address public constant SWAP_ROUTER = address(0x3bFA4769FB09eefC5a80d6E87c3B9C650f7Ae48E); 

    // Token addresses
    IERC20 public constant USDC = IERC20(0x1c7D4B196Cb0C7B01d743Fbc6116a902379C7238);
    address public constant WETH = address(0xfFf9976782d46CC05630D1f6eBAb18b2324d6B14);
    ...
    address public constant USDC_WHALE = address(0x75C0c372da875a4Fc78E8A37f58618a6D18904e8);
    ...
    function setUp() public override {
        ...
        // Fork at block 9669199
        vm.createSelectFork(vm.envString("SEPOLIA_RPC_URL"), 9669199);
    ...
    /// @dev Swaps USDC for WETH using Uniswap V3 on Sepolia testnet
    function test_swapUSDCtoWETH() public {
        // Use 2 USDC: 1 USDC fee + 1 USDC swap (matching your successful on-chain tx)
        uint256 amountIn = 2 * 1e6; // 2 USDC
        ...
        // Execute swap: USDC -> WETH with 0.05% fee (based on actual Sepolia pool)
        ddca.swapByDelegation(
            delegations_,
            address(USDC),  // tokenIn
            WETH,           // tokenOut
            FEE_LOW,        // 0.05% fee tier (500) - the pool that exists on Sepolia
            amountIn
        );
